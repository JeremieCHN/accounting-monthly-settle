...
export function exportResult(result: CalcResult): void {
  ...
  // ---- Sheet 1: 期末库存 ----
  // 列头: 物料名称、数量、含税单价、不含税单价、税率、含税金额
  const closingData: unknown[][] = [
    ['物料名称', '数量', '含税单价', '不含税单价', '税率', '含税金额'],
  ];
  let totalClosingQty = 0;
  let totalClosingAmount = 0;

  for (const m of result.materials) {
    const exTax = exTaxPrice(m.closingAvgPrice, m.closingTaxRate);
    closingData.push([
      m.materialName,
  ...
  wsClosing['!cols'] = [{ wch: 20 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 8 }, { wch: 14 }];
